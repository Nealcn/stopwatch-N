...
from .protocol import (
    ...
    parse_audio_frame, parse_state_event, parse_mouse_event,
    AudioFrame, StateEvent, MouseEvent,
)
...
class BleClient:
    def __init__(self):
        ...
        self.on_mouse_event: Optional[Callable[[MouseEvent], None]] = None
    ...
    def _on_state_notify(self, sender, data: bytearray):
        raw = bytes(data)
        # 触摸板鼠标帧（扩展）：先尝试 mouse 解析，失败再当状态事件
        mouse = parse_mouse_event(raw)
        if mouse:
            if self.on_mouse_event:
                self.on_mouse_event(mouse)
            return
        logger.info("收到状态通知: %d 字节: %s", len(raw), raw[:120])
        event = parse_state_event(raw)
        if event:

...
class FloatingBallWindow(QWidget):
    ...
    def __init__(self):
        ...
        # 显示器插拔/分辨率变化时重新钳制，避免球留在已移除的屏幕区域
    ...
    def _relayout(self):
        ...
        if not self._has_text:
            return
        s = self.screen()
        scr = s.availableGeometry() if s else self._virtual_desktop()
    ...
    def _init_pos(self):
        if self._saved_x != -1:  # -1 表示从未保存过位置
    ...
    def _virtual_desktop(self):
        ...
        for scr in QApplication.screens():
            desk = desk.united(scr.availableGeometry())
        return desk

    def _clamped_pos(self, x, y):
        """把坐标钳制到虚拟桌面内，保证窗口完整可见"""
        desk = self._virtual_desktop()
        x = max(desk.left(), min(x, desk.right() - self.width() + 1))
        y = max(desk.top(), min(y, desk.bottom() - self.height() + 1))
        return QPoint(x, y)

    def _clamp(self):
        p = self._clamped_pos(self.x(), self.y())
        if p != self.pos():
            self.move(p)
        self.position_changed.emit()

    def _on_screens_changed(self, *args):
        """显示器插拔/分辨率变化：把悬浮球拉回当前可见区域"""
        self._clamp()
        if self._has_text:
            self._relayout()
            self._clamp()
    ...
    def mouseMoveEvent(self, e):
        ...
        if delta.manhattanLength() > CLICK:
            ...
            self.move(self._clamped_pos(target.x(), target.y()))
            self._drag_pt = e.globalPos()
            # 拖动时实时更新布局（四象限自适应翻转）
            if self._has_text:
                self._relayout()
